...
4.  **Support Features Porting:** Integrate specific support components from `solo-support` into the `supportsheep` chassis (Search-centric layout, AI Chatbot, Voice Widget, "Was this helpful" feedback).
...
## Architecture
*   **Foundation:** A direct clone of the `pooriaarab/supportsheep` repository.
*   **Hosting:** Cloudflare Pages (Frontend) and Cloudflare Workers (Backend API/Routing if applicable based on supportsheep).
*   **Database:** Matches `supportsheep`'s existing DB architecture (likely D1 or similar Cloudflare native DB) for managing tenant settings, users, and markdown content.
*   **Frontend:** React/Next.js (matching supportsheep).

## Implementation Strategy (Approach 1: supportsheep First)
1.  **Clone & Rebrand:** Clone `supportsheep`. Find and replace all instances of "supportsheep" to "supportsheep". Update favicons, metadata, and SEO tags.
