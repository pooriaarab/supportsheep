...
describe("classifyRequestTenant", () => {
  it("classifies a resolved tenant as a blog", () => {
    expect(classifyRequestTenant("acme.blogbat.com", "b-acme")).toEqual({
      kind: "blog",
      blogId: "b-acme",
    });
  ...
  it("classifies marketing hosts as marketing", () => {
    expect(classifyRequestTenant("blogbat.com", null)).toEqual({
      kind: "marketing",
    });
    expect(classifyRequestTenant("www.blogbat.com", null)).toEqual({
      kind: "marketing",
    });
  ...
  it("classifies an unknown *.blogbat.com tenant subdomain as not-found", () => {
    expect(classifyRequestTenant("ghost.blogbat.com", null)).toEqual({
      kind: "not-found",
    });
  });
  ...
  it("keeps first-party platform hosts on marketing/default (never 404)", () => {
    ...
    expect(classifyRequestTenant("customers.blogbat.com", null)).toEqual({
      kind: "marketing",
    });
    expect(classifyRequestTenant("app.blogbat.com", null)).toEqual({
      kind: "marketing",
    });
    expect(classifyRequestTenant("staging.blogbat.com", null)).toEqual({
      kind: "marketing",
    });
  });
