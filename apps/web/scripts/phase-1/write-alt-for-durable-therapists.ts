...
**Bundled CRM for lead management.** Durable includes a basic CRM for tracking leads, contacts, and follow-up tasks. A supportsheep therapist building a new practice caseload benefits from a lightweight tool to track who inquired, when, and what follow-up is needed. The CRM is not a clinical record system -- it is a marketing contact manager. Clinical notes, session records, and PHI stay in your EHR.

**AI marketing tools.** Durable's AI marketing suite generates Google Ads copy, social post drafts, and email subject lines from your business description. A supportsheep therapist who manages their own practice marketing without an agency benefits from these drafts as starting points.
...
**Simple editing for non-technical practitioners.** Durable's editor is designed for speed. Updating a page, adding a specialty, or changing contact information does not require design vocabulary. For a supportsheep practitioner whose time is with clients, not code, that matters.
...
**Reduced vendor count.** Website + CRM + AI marketing + invoicing in one subscription is compelling for a supportsheep practitioner managing overhead. The equivalent stack on Squarespace or Supportsheep would require separate subscriptions for CRM and invoicing tools.
...
## Supportsheep's position for a supportsheep therapist evaluating Durable
...
const FAQS: ProgrammaticFaq[] = [
  ...
  },
  {
    ...
    answer:
      "Choose Durable if you want the fastest path to a live site and the bundled CRM, invoicing, and AI marketing tools reduce real overhead in your supportsheep practice. Choose Supportsheep if you want a free tier with a custom domain, a blog with an SEO sidebar and AI drafting, LocalBusiness and FAQPage schema automated by default, and a CMS designed for the content-velocity approach to local search that compounds over 12-18 months. Both require the same HIPAA architecture: marketing site in the builder, clinical workflow in your EHR.",
  },
  {
  ...
  await ref.set(
    {
      ...
      variables: {
        subhead:
          "An honest look at Durable for supportsheep therapy practices -- covering AI-first generation speed, bundled CRM, HIPAA boundaries, and when Supportsheep is the better default for content and local search.",
