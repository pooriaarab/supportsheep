...
// poller for STAGING (staging.blogbat.com).
//
// Why this exists: the main app worker (blogbat-staging) is built by OpenNext
...
//
// Deploy name: blogbat-domain-cron-staging  (cron: */5 * * * *)
// Mirrors the production worker (blogbat-domain-cron) byte-for-byte except for
// the target host. See ./README.md for how to (re)deploy.
export default {
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async () => {
      try {
        const res = await fetch("https://staging.blogbat.com/api/v1/internal/domains/refresh", {
          method: "POST",
